...
MagAlphaGen8::MagAlphaGen8(){
}

uint16_t MagAlphaGen8::readAngleRaw16(){
    uint16_t angle;
...
uint8_t MagAlphaGen8::readAngleRaw8(){
    uint8_t angle;
    digitalWrite(_spiChipSelectPin, LOW);
...
uint16_t MagAlphaGen8::readAngleRaw(bool* error){
    uint16_t angle;
    uint8_t parity;
...
uint8_t MagAlphaGen8::readRegister(uint8_t address){
...
uint8_t MagAlphaGen8::writeRegister(uint8_t address, uint8_t value){
    uint8_t readbackRegisterValue;
    digitalWrite(_spiChipSelectPin, LOW);
...
void MagAlphaGen8::readRegisterBurst(uint8_t address, uint8_t valueArray[], uint16_t numberOfRegister){
    uint8_t readbackRegisterValue;
    digitalWrite(_spiChipSelectPin, LOW);
    _spi->transfer16(READ_REG_COMMAND | (((address) & 0x1F) << 8) | 0x00);
    digitalWrite(_spiChipSelectPin, HIGH);
...
void MagAlphaGen8::writeRegisterBurst(uint8_t address, uint8_t valueArray[], uint16_t numberOfRegister){
...
/*
uint16_t MagAlphaGen8::detectSensorGeneration(){
    uint16_t chipId;
    digitalWrite(_spiChipSelectPin, LOW);
...
uint16_t MagAlphaGen8::getZero()
{
    return (readRegister(1)<<8) + readRegister(0);
}

void MagAlphaGen8::setZero(uint16_t zero){
    writeRegister(0, zero&0xFF);
    writeRegister(1, zero>>8);
}

uint16_t MagAlphaGen8::setCurrentAngleAsZero(){
    uint16_t zero;
    setZero(0);
    zero=readAngleRaw16();
    setZero(zero);
...
void MagAlphaGen8::restoreAllRegisters(){
    digitalWrite(_spiChipSelectPin, LOW);
...
void MagAlphaGen8::storeAllRegisters(){
    digitalWrite(_spiChipSelectPin, LOW);
...
void MagAlphaGen8::storeRegister(uint8_t address){
    digitalWrite(_spiChipSelectPin, LOW);
...
void MagAlphaGen8::clearErrorFlags(){
    digitalWrite(_spiChipSelectPin, LOW);
...
uint16_t MagAlphaGen8::getNumberOfRegisters(){
    return 32;
}

void MagAlphaGen8::getLockedRegisters(uint8_t unlockedRegisters[]){
...
void MagAlphaGen8::getPartNumber(char *partNumber){
    uint8_t partId = readRegister(REG_PID);
    switch(partId) {
        case 1:
            sprintf(partNumber, "MA980");
            break;
        case 2:
            sprintf(partNumber, "MA981");
            break;
        default:
            sprintf(partNumber, "Gen8 [%u] Unknown Part Number", partId);
    }
}

uint8_t MagAlphaGen8::getSiliconId(){
    return (readRegister(REG_SID)  & 0xF0) >> 4;
}

uint8_t MagAlphaGen8::getSiliconRevision(){
    return readRegister(REG_SID) & 0x0F;
}

uint8_t MagAlphaGen8::getRegisterMapRevision(){
